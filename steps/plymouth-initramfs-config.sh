...
if [ ! -f "${CONFIG_FILE}" ]; then
  log_error "plymouth-initramfs-config: config.txt not found: ${CONFIG_FILE}"
  exit 1
fi
...
if [ ! -f "${initrd_path}" ]; then
  ...
  log_warn "plymouth-initramfs-config: leaving config.txt unchanged (auto_initramfs not modified)"
  exit 0
fi
