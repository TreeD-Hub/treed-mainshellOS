...
if [ -z "${CONFIG_FILE:-}" ] || [ ! -f "${CONFIG_FILE}" ]; then
  CONFIG_FILE="$(detect_config_file "${BOOT_DIR}" 2>/dev/null || true)"
fi
...
if [ -n "${CONFIG_FILE:-}" ] && [ -f "${CONFIG_FILE}" ]; then
  ...
  failf "config.txt (${CONFIG_FILE:-unset} missing)"
fi
...
' "${CMDLINE_CONTENT}" | grep -q "plymouth.enable=0"; then
  ...
  failf "cmdline file (${CMDLINE_FILE:-unset} missing)"
fi

MASK_TTY1="${TREED_MASK_TTY1:-1}"
case "${MASK_TTY1}" in
  0|false|FALSE|no|NO|off|OFF)
    pass "getty@tty1 mask check skipped (TREED_MASK_TTY1=${MASK_TTY1})"
    ;;
  *)
...
gm="$(grep -E "^gpu_mem=" "${CONFIG_FILE:-/dev/null}" 2>/dev/null | tail -n1 | cut -d= -f2)"
